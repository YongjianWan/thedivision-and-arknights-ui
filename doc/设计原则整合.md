...
通过 `usePerformanceMode()` hook 自动检测设备能力并选择模式：

| 效果 | full | reduced | minimal |
|------|------|---------|---------|
| 背景模糊 (backdrop-filter) | ✓ 实时 | ✗ 用静态图 | ✗ 纯色 |
| 全局噪点 (NoiseOverlay) | ✓ | ✗ | ✗ |
| 粒子数量 | 12 | 6 | 0 |
| 构建式动画 | ✓ 完整序列 | ✗ 简化淡入 | ✗ 直接显示 |
| 复杂动画 | ✓ | ✓ | ✗ |
...
```tsx
import { usePerformanceMode } from '@/components';

function MyComponent() {
  const { mode, enableBlur, particleCount, enableNoise } = usePerformanceMode();
...
| Not found | TARGET NOT FOUND (NULL_PTR) |
...
```tsx
import { CLINICAL_STATUS, CLINICAL_ACTIONS } from '@/components';
...
危险操作（删除、重置等）不要用弹窗确认，用 **长按 1.5 秒填满进度条** 的方式。
...
```tsx
import { HoldButton, HoldToDelete } from '@/components';

// 自定义
<HoldButton onConfirm={handleDelete} holdDuration={2000}>
  HOLD TO CONFIRM
</HoldButton>
...
展示"变化的过程"而非仅"状态的快照"。用 SVG 线框 + 流动光点模拟数据传输。

### 组件

- `Schematic` - 基础原理图，自定义节点和连线
- `ServerTopology` - 预设：服务器拓扑图（中心辐射）
- `DataFlow` - 预设：数据流图（线性阶段）

### 使用方式

```tsx
import { ServerTopology, DataFlow } from '@/components';

// 服务器拓扑
<ServerTopology
  servers={[
    { id: 'a', label: 'NODE_A', status: 'online' },
    { id: 'b', label: 'NODE_B', status: 'busy' },
  ]}
/>
